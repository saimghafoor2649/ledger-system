...
app.post("/Customerform", (req, res) => {
  ...
  db.query(
    sql,
    [req.body.customerid, req.body.customername, req.body.customerphoneno],
    (err, result) => {
      if (err) {
        console.log("Database error:", err);
        return res.status(500).json({ error: "Database error" });
...
app.put("/Customerform/:id", (req, res) => {
  ...
  const sql =
    "UPDATE customerinfo SET customername = ?, customerphoneno = ? WHERE customerid = ?";
...
app.put("/Productform/:id", async (req, res) => {
  ...
  try {
    const result = db.query(
      "UPDATE productinfo SET productname = ? WHERE productid = ?",
      [productname, id]
    );
    if (err) {
      console.log("Database error:", err);
